...
export default function StockReceived() {
  ...
  return (
            ...
            <>
              {selectedStation?.code === 'MUTT' && (
                <Button
                  ...
                  onClick={() => { setShowDepotForm(true); setShowDepotAddForm(false); loadDepotLogs(); setError(''); }}
    ...
    >
      ...
      {/* MUTT SC ONLY: External Transfer Log + Form Modal */}
      {selectedStation?.code === 'MUTT' && role === ROLES.SC && (
      <Modal
        isOpen={showDepotForm}
        onClose={() => { setShowDepotForm(false); setShowDepotAddForm(false); setError(''); }}
        title="External Transfer — Depot / CCR"
        size="lg"
        footer={
          <>
            <Button variant="outline" onClick={() => { setShowDepotForm(false); setShowDepotAddForm(false); setError(''); }}>Close</Button>
            {showDepotAddForm && (
      ...
      >
        ...
        <div style={{ marginBottom: '16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
            <h4 style={{ margin: 0, fontSize: '14px', fontWeight: 700, color: 'var(--color-gray-700)' }}>🏭 External Transfer History</h4>
            <Button
              variant={showDepotAddForm ? 'outline' : 'warning'}
              size="sm"
              onClick={() => { setShowDepotAddForm(v => !v); setError(''); }}
            >
              {showDepotAddForm ? '✕ Cancel New Transfer' : '+ New Transfer'}
            </Button>
          </div>

          {/* ── Add Transfer Form (collapsible) ──────────────────────── */}
          {showDepotAddForm && (
            <div style={{ border: '1px solid var(--color-warning-300)', borderRadius: '8px', padding: '16px', marginBottom: '16px', backgroundColor: 'var(--color-warning-50)' }}>
          ...
          {depotLogsLoading ? (
            <div style={{ textAlign: 'center', padding: '24px', color: 'var(--color-gray-400)' }}>Loading history…</div>
          ) : depotLogs.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '24px', color: 'var(--color-gray-400)' }}>No external transfers recorded yet.</div>
          ) : (
            <div style={{ maxHeight: '320px', overflowY: 'auto', border: '1px solid var(--color-border)', borderRadius: '8px' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '13px' }}>
                <thead>
                  <tr style={{ background: 'var(--color-gray-100)', position: 'sticky', top: 0, zIndex: 1 }}>
                    <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 700, color: 'var(--color-gray-600)', borderBottom: '2px solid var(--color-border)', background: 'var(--color-gray-100)' }}>Date</th>
                    <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 700, color: 'var(--color-gray-600)', borderBottom: '2px solid var(--color-border)', background: 'var(--color-gray-100)' }}>Station</th>
                    <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 700, color: 'var(--color-gray-600)', borderBottom: '2px solid var(--color-border)', background: 'var(--color-gray-100)' }}>Item</th>
                    <th style={{ padding: '8px 12px', textAlign: 'right', fontWeight: 700, color: 'var(--color-gray-600)', borderBottom: '2px solid var(--color-border)', background: 'var(--color-gray-100)' }}>Qty</th>
                    <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 700, color: 'var(--color-gray-600)', borderBottom: '2px solid var(--color-border)', background: 'var(--color-gray-100)' }}>Destination</th>
                    <th style={{ padding: '8px 12px', textAlign: 'left', fontWeight: 700, color: 'var(--color-gray-600)', borderBottom: '2px solid var(--color-border)', background: 'var(--color-gray-100)' }}>Remarks</th>
                  </tr>
                </thead>
                <tbody>
                  {depotLogs.map((log, i) => {
                    ...
                    const dispUnit = getDisplayUnit(unit);
                    const dispVal = toDisplayValue(log.quantity_used, unit);
                    const qty = dispUnit === 'Nos' ? `${Math.round(dispVal)} Nos` : `${dispVal.toFixed(2)} ${dispUnit}`;
                    // Parse destination and extra remark from stored format:
                    // "Depot Transfer Out - to Depot" or "Depot Transfer Out - to CCR - user note"
                    const remarkStr = log.remarks || '';
                    const dest = remarkStr.includes('to CCR') ? 'CCR' : 'Depot';
                    const extraMatch = remarkStr.match(/to (?:Depot|CCR)\s*-\s*(.*)/);
                    const extraRemark = extraMatch ? extraMatch[1].trim() : '';
                    return (
                      <tr key={log.id} style={{ borderBottom: '1px solid var(--color-gray-100)', background: i % 2 === 0 ? 'transparent' : 'var(--color-gray-50)' }}>
                        <td style={{ padding: '7px 12px', whiteSpace: 'nowrap' }}>{formatDate(log.consumption_date)}</td>
                        <td style={{ padding: '7px 12px', fontWeight: 600, color: 'var(--color-primary-700)' }}>{log.stations?.code ?? '—'}</td>
                        <td style={{ padding: '7px 12px' }}>{log.inventory_items?.name ?? '—'}</td>
                        <td style={{ padding: '7px 12px', textAlign: 'right', fontWeight: 600 }}>{qty}</td>
                        <td style={{ padding: '7px 12px' }}>
                          <span style={{ padding: '2px 8px', borderRadius: '12px', fontSize: '11px', fontWeight: 700, background: dest === 'CCR' ? 'var(--color-primary-100)' : 'var(--color-warning-100)', color: dest === 'CCR' ? 'var(--color-primary-700)' : 'var(--color-warning-700)' }}>
                            {dest === 'CCR' ? '🏢 CCR' : '🏭 Depot'}
                          </span>
                        </td>
                        <td style={{ padding: '7px 12px', color: 'var(--color-gray-500)', fontSize: '12px' }}>{extraRemark || '—'}</td>
                      </tr>
                    );
                  })}
                </tbody>
